 ...
 * - Description fields use MarkdownInput which starts in preview mode —
 *   must click "Edit" to reveal the textarea before filling
...
import {
  isIconCheckboxCheckedByLabel,
  setIconCheckboxByLabel
} from './checkbox';
 ...
 * Description uses MarkdownInput (preview mode by default).
 */
export async function updateCfDescription(
  page: Page,
  description: string
): Promise<void> {
 ...
 * Assumes we're already on the CF general page.
 */
export async function updateCfName(
  page: Page,
  name: string
): Promise<void> {
...
async function findConditionCardByName(
  page: Page,
  conditionName: string
): Promise<Locator> {
  const inputs = page.locator('input[placeholder="Condition name"]');
...
export async function hasConditionByName(
  page: Page,
  conditionName: string
): Promise<boolean> {
  const inputs = page.locator('input[placeholder="Condition name"]');
...
export async function removeConditionByName(
  page: Page,
  conditionName: string
): Promise<void> {
 ...
 */
export async function getConditionValueByName(
  page: Page,
  conditionName: string
): Promise<string> {
  const card = await findConditionCardByName(page, conditionName);
 ...
 */
export async function getConditionTypeByName(
  page: Page,
  conditionName: string
): Promise<string> {
  const card = await findConditionCardByName(page, conditionName);
...
export async function addEnumCondition(
  ...
  const card = confirmButton.locator(
    'xpath=ancestor::div[contains(@class,"relative")]'
  );
 ...
 * Assumes we're already on the CF general page.
 */
export async function updateCfIncludeInRename(
  page: Page,
  enabled: boolean
): Promise<void> {
 ...
 */
export async function getCfFieldValue(
  page: Page,
  id: string
): Promise<string> {
 ...
 * Assumes we're already on the CF general page.
 */
export async function addCfTag(
  page: Page,
  tag: string
): Promise<void> {
 ...
 */
export async function removeCfTag(
  page: Page,
  tag: string
): Promise<void> {
...
export async function goToRegex(
  page: Page,
  databaseId: number,
  name: string
): Promise<void> {
 ...
 */
export async function updateRegexDescription(
  page: Page,
  description: string
): Promise<void> {
  await fillMarkdownInput(page, 'description', description);
